...
We are also experimenting a tool to generate new `obsidian` project. The generated project should include some default configuration such as database connection config by default because we found that setting up database connection everytime for a new project is quite annoying. We want our users focus on the important stuffs without sweating the small things.
